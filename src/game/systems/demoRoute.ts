...
export function buildDemoRoute(layout: CityLayout): DemoBeat[] {
  ...
  return [
    ...
    },
    {
      ...
      topics: ["무기"],
      /*
       * 싸움을 끝내고 대장에게 달려간다 — 55m를 10초에.
       *
       * 8초였다. 대장이 교차로에서 **공사장 한가운데**로 옮겨 가면서 이 구간이
       * 39m에서 55m로 늘었고, 8초로는 달리기 최고 속도의 92%가 필요해졌다.
       * 대본은 최고 속도의 80%까지만 쓴다(`DEMO_PACE`) — 실제로 그렇게 못 달린다.
       */
      mode: "run",
      x: -78,
      z: 84,
