...
Meshy 입력 정본은 `assets/concepts/meshy/`에 있다.

**이름과 사연은 바꾸지 않았다.** 여기에는 「반입 때 `roster.ts`의 이름·사연도
외형에 맞게 바꾼다」고 적혀 있었는데, 실제로 넷을 놓고 보니 사연은 종(種)이 아니라
**태어난 자리**에 대한 이야기였다 — 「가로등이 나가던 골목에서 혼자 켜져 있었다」는
버섯이어도 그대로 성립한다. 도깨비는 무엇이든 그 모습을 할 수 있다.

짝은 색으로 맞췄다(`dokebi/companionShapes.ts`): 그을음(#57505f)은 검은 고양이,
물비늘(#2f9fd4)은 청록 패딩을 입은 흰곰, 초롱은 크림 몸에 잎 싹을 단 버섯,
자정은 아직 안 온 로봇이다.

**동료는 이제 땅을 걷는다.** 예전에는 1.7m 상공에 떠 있는 등불이었고 그것이
TRAILER 8.2의 실루엣 원칙이었는데, 걷는 동작을 가진 두 발 생물에게는 성립하지
않는다 — 공중에서 다리를 저으면 연출이 아니라 버그로 읽힌다.
...
지금 외부 에셋은 아홉이다 — `character.glb`(1.7MB), `title-street.webp`(137KB),
배경 차량 셋(`public/models/traffic-*.glb`, 103~122KB), 동료 셋
(`public/models/companion-*.glb`, 330~436KB), 미니 보스
(`public/models/boss-scrap-foreman.glb`, 740KB). 남은 동료는 로봇 하나다.

**타는 것은 여기 없다.** 한때 여우와 카트를 넣었다가 뺐다 — 받은 파일이
아니라 Downloads에 함께 있던 **다른 프로젝트(kart-universe)의 것**이었다.
날짜가 두 달 어긋나 있었다.
...
|---|---|---|---|
| 1 | ~~**동료 4종**~~ | `dokebi/` — 셋 **배선 완료** (2026-08-26), 로봇은 등불로 남음 | 3~6k / 512² |
| 2 | ~~**플레이어**~~ | `public/character.glb` **교체 완료** (2026-08-26). 몸·동작 모두 `Meshy_AI_Meshy_Merged_Animations (5).glb` 한 파일에서 온다 — 한때 몸만 `Junior_Gadgeteer_biped.zip`에서 가져왔더니 그 모델이 **블래스터를 들고 있어** 주인공이 늘 총을 든 채였다 | 10,424 / 1024² |
