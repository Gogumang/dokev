...
describe("같은 설정을 같은 이름으로 부르는가", () => {
  ...
  it("저감 모션의 이름이 두 화면에서 같다", () => {
    const match = title.match(/label="([^"]*모션[^"]*)"/);
    expect(match, "시작 화면에 모션 설정이 없다").not.toBeNull();
    const name = match?.[1] ?? "";
    expect(hud, `시작 화면은 「${name}」인데 HUD가 다른 말을 쓴다`).toContain(name);
  });

  it("소리의 이름도 두 화면에서 같다", () => {
    /*
     * 「사운드」 ↔ 「소리」로 갈려 있었다. 모션과 똑같은 실수를 두 번 한 것이라,
     * 한쪽만 고치고 끝내면 다음에 또 갈린다.
     */
    const match = title.match(/label="(소리|사운드)"/);
    expect(match, "시작 화면에 소리 설정이 없다").not.toBeNull();
    expect(hud, `시작 화면은 「${match?.[1]}」`).toContain(match?.[1] ?? "");
  });
});
