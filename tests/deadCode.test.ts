...
describe("테스트가 살려 두는 죽은 코드", () => {
  ...
  const KEPT: Record<string, string> = {
    ...
    DEMO_PACE:
      "시연 코스가 실제로 가능한지 재는 잣대(최고 속도의 몇 %를 실제로 내는가). " +
      "화면에는 필요 없다 — 대본이 지켜야 하는 약속이고, 그 약속을 검사가 이 값으로 잰다",
    beatSpeed:
      "시연 코스의 한 구간에 필요한 속도. 같은 이유로 제품이 쓰지 않는다 — " +
      "「자전거로 12초에 200m」 같은 대본을 막는 것이 이 함수의 유일한 일이다",
    RETIRED_WEAPONS:
      "주인공이 들지 않기로 한 무기 목록. 제품은 `WEAPON_ORDER`만 보므로 부르는 곳이 없다 — " +
      "그래도 남긴다: 「어느 쪽에도 없는 정의는 실수다」를 검사가 이 목록으로 가른다. " +
      "지우면 은퇴가 「빠뜨린 것」과 구분되지 않는다",
    hintRows:
