...
describe("public에 에셋이 들어오지 않는가", () => {
  ...
  const ALLOWED = new Set([
     ...
     *
     * 원본은 동작 열한 개가 붙은 10.6MB였다. 지금 남은 것은 **코드가 실제로
     * 트는 셋**뿐이다(걷기·달리기·공격). 나머지는 핸드스탠드 플립·펀치 콤보·
     * 일어서기처럼 **대응하는 상태가 게임에 없는** 것들이고, 애니메이션이
     * 파일의 절반이라 예산을 그것들이 먹고 있었다.
     *
     * 「쓰는 것만 남는다」는 `tests/companionShapes.test.ts`가 양방향으로
     * 지킨다 — 코드가 부르는 이름이 파일에 있는지, 파일에 있는데 안 부르는
     * 것이 없는지.
     */
  ...
  it("동료와 보스가 예산을 넘지 않는다", () => {
    /*
     * GLB 하나 600KB — ASSET_PLAN 4절. 동료는 330~436KB, 대장은 758KB다
     * (안 쓰는 동작을 버려 275KB를 돌려받았다).
     *
